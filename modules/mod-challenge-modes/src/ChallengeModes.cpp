...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        ...
        player->KillPlayer();
        player->GetSession()->KickPlayer("硬核角色已死亡");
    }
    ...
    void OnPlayerKilledByCreature(Creature* /*killer*/, Player* player) override
    {
        ...
        for (uint8 i = 0; i < EQUIPMENT_SLOT_END; ++i)
        {
            if (Item* pItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            {
                ...
                ChatHandler(player->GetSession()).PSendSysMessage("|cffDA70D6你失去了 |cffffffff|Hitem:%d:0:0:0:0:0:0:0:0|h[%s]|h|r", pItem->GetEntry(), pItem->GetTemplate()->Name1.c_str());
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "启用低品质物品模式", 0, SETTING_ITEM_QUALITY_LEVEL);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "启用缓慢经验模式", 0, SETTING_SLOW_XP_GAIN);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "启用仅任务经验模式", 0, SETTING_QUEST_XP_ONLY);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "启用钢铁侠模式", 0, SETTING_IRON_MAN);
        }
